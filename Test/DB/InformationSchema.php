...
class Test_DB_InformationSchema extends SabelTestCase
{
  ...
  public function testUse()
  {
    ...
    $this->assertEquals($name->type, Sabel_DB_Schema_Const::STRING);
    $this->assertEquals((int)$name->max, 128);
    $this->assertTrue($name->notNull);
    $this->assertFalse($name->primary);

    $this->assertEquals($status->type, Sabel_DB_Schema_Const::BOOL);
    $this->assertTrue($status->notNull);
    $this->assertFalse($status->primary);

    $this->assertEquals($comment->type, Sabel_DB_Schema_Const::STRING);
    ...
    $this->assertEquals($pare_id->type, Sabel_DB_Schema_Const::INT);
    ...
    $this->assertEquals($birth->type, Sabel_DB_Schema_Const::DATE);
    ...
    $this->assertEquals($time->type, Sabel_DB_Schema_Const::TIMESTAMP);
    $this->assertEquals($com->type, Sabel_DB_Schema_Const::TEXT);
  }
}
