...
class Sabel
{
  public static function initializeApplication()
  {
    ...
    if (ENVIRONMENT !== 'development' && is_readable($cacheFilepath)) {
      $c->setClasses(unserialize(file_get_contents($cacheFilepath)));
      require (SABEL_CLASSES);
    } else {
      ClassFileStructureReader::create('sabel/')
        ->read()
        ->write(SABEL_CLASSES);
        
      if (!defined('TEST_CASE')) require (SABEL_CLASSES);
      ...
      file_put_contents($cacheFilepath, serialize($c->getClasses()));
    }
  }
}
...
class CacheAuto
{
  ...
  public function __construct($key, $path = null)
  {
    if (extension_loaded('apc')) {
      $this->implement = new CacheApc($key);
    } else if (!is_null($path)){
      $this->implement = new CacheFile($path);
    } else {
      throw new Exception('no auto cache');
    }
  }
  
  public static function create($key, $path)
  {
    if (is_null(self::$instance)) self::$instance = new self($key);
    return self::$instance;
  }
  
  public function read()
  {
    return $this->implement->read();
  }
  
  public function write($value)
  {
    return $this->implement->write($value);
  }
  
  public function isReadble()
  {
    return $this->implement->isReadable();
  }
