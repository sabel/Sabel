...
class Sabel_DB_Driver_Pgsql implements Sabel_DB_Driver_Interface
{
  ...
  public function executeInsert($table, $data, $defColumn)
  {
    ...
    $this->stmtFlag = Sabel_DB_Driver_PgsqlStatement::exists(implode('', $sql), $data);
  ...
  {
    $this->stmtFlag = Sabel_DB_Driver_PgsqlStatement::exists($this->sqlObj->getSQL(), $conditions, $constraints);
  ...
  public function execute($sql = null, $param = null)
  {
    try {
      if (isset($sql)) {
        if (!($this->result = pg_query($this->conn, $sql))) {
          throw new Exception('Error: Sabel_DB_Driver_Pgsql::pg_query()');
        } else {
          return true;
        }
      } elseif ($this->stmtFlag) {
        $stmtName = Sabel_DB_Driver_PgsqlStatement::get();
        ...
        $stmtName = Sabel_DB_Driver_PgsqlStatement::incStmtName();
    ...
    try {
      if (!($this->result = pg_execute($this->conn, $stmtName, $this->param))) {
        throw new Exception('Error: Sabel_DB_Driver_Pgsql::execute()');
        return false;
      } else {
        $tmp = $this->param;
        $this->param = array();
...
class Sabel_DB_Driver_PgsqlStatement
{
  private static $stmt;
  private static $sql;
