...
/**
 * A Sabel_Http_CookieJar object is designed to contain and maintain HTTP cookies, and should
 * be used along with Sabel_Http_Client in order to manage cookies across HTTP requests and
 * responses.
 *
 * The class contains an array of Sabel_Http_Cookie objects. Cookies can be added to the jar
 ...
 * can be returned either in their native form (as Sabel_Http_Cookie objects) or as strings -
 ...
 * (by passing Sabel_Http_CookieJar::COOKIE_STRING_ARRAY) or one unified string for all cookies
 * (by passing Sabel_Http_CookieJar::COOKIE_STRING_CONCAT).
 *
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Sabel_Http_CookieJar  /* Sabel_Http_CookieJar */
{
    /**
     * Return cookie(s) as a Sabel_Http_Cookie object
     *
     */
    ...
    /**
     * Add a cookie to the jar. Cookie should be passed either as a Sabel_Http_Cookie object
     * or as a string - in which case an object is created from the string.
     *
     * @param Sabel_Http_Cookie|string $cookie
     * @param Sabel_Http_Uri|string    $ref_uri Optional reference URI (for domain, path, secure)
     */
    public function addCookie($cookie, $ref_uri = null)
    {
        ...
        if ($cookie instanceof Sabel_Http_Cookie) {
            ...
            throw new Sabel_Exception_Runtime('Supplient argument is not a valid cookie string or object');
        }
    }
     ...
     * to the cookie jar.
     *
     * @param Sabel_Http_Response $response
     * @param Sabel_Http_Uri|string $ref_uri Requested URI
     */
    public function addCookiesFromResponse($response, $ref_uri)
    {
        if (! $response instanceof Sabel_Http_Response) {
            throw new Sabel_Exception_Runtime('$response is expected to be a Response object, ' .
     ...
     *
     * @param int $ret_as Whether to return cookies as objects of Sabel_Http_Cookie or as strings
     * @return array|string
     */
     ...
     *
     * @param string|Sabel_Http_Uri $uri URI to check against (secure, domain, path)
    ...
    {
        if (is_string($uri)) $uri = new Sabel_Http_Uri($uri);

        if (! $uri instanceof Sabel_Http_Uri) {
            throw new Sabel_Exception_Runtime("Invalid URI string or object passed");
        }
     ...
     *
     * @param Sabel_Http_Uri|string $uri The uri (domain and path) to match
     * @param string $cookie_name The cookie's name
     * @param int $ret_as Whether to return cookies as objects of Sabel_Http_Cookie or as strings
     * @return Sabel_Http_Cookie|string
     */
    public function getCookie($uri, $cookie_name, $ret_as = self::COOKIE_OBJECT)
    {
        ...
        if (! $uri instanceof Sabel_Http_Uri) {
            throw new Sabel_Exception_Runtime('Invalid URI specified');
        }
        ...
        if (isset($this->cookies[$uri->getHost()][$path][$cookie_name])) {
            ...
            switch ($ret_as) {
                ...
                default:
                    throw new Sabel_Exception_Runtime("Invalid value passed for \$ret_as: {$ret_as}");
                    break;
            }
        } else {
            return false;
     ...
     *
     * @param Sabel_Http_Cookie|array $ptr
     ...
     * of the cookie.
     *
     * @param Sabel_Http_Response $response HTTP Response object
     * @param Sabel_Http_Uri|string $uri The requested URI
     * @return Sabel_Http_CookieJar
     * @todo Add the $uri functionality.
     */
    public static function fromResponse(Sabel_Http_Response $response, $ref_uri)
    {
        $jar = new self();
