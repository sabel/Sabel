...
class Sabel_Http_Client_Adapter_Socket implements Sabel_Http_Client_Adapter_Interface
{
    ...
    public function connect($host, $port = 80, $secure = false)
    {
        ...
        if (! is_resource($this->socket) || ! $this->config['keepalive']) {
            ...
            if ($secure) {
                if ($this->config['sslcert'] !== null) {
                    if (! stream_context_set_option($context, 'ssl', 'local_cert',
                                                    $this->config['sslcert'])) {
                        throw new Sabel_Exception_Runtime('Unable to set sslcert option');
                    }
                }
                if ($this->config['sslpassphrase'] !== null) {
                    if (! stream_context_set_option($context, 'ssl', 'passphrase',
                                                    $this->config['sslpassphrase'])) {
                        throw new Sabel_Exception_Runtime('Unable to set sslpassphrase option');
                    }
                }
            }
            ...
            if (! stream_set_timeout($this->socket, (int) $this->config['timeout'])) {
                throw new Sabel_Exception_Runtime('Unable to set the connection timeout');
            }
    ...
    public function write($method, $uri, $http_ver = '1.1', $headers = array(), $body = '')
    {
        ...
        if (! $this->socket) {
            throw new Sabel_Exception_Runtime('Trying to write but we are not connected');
        }

        $host = $uri->getHost();
        $host = (strtolower($uri->getScheme()) == 'https' ? $this->config['ssltransport'] : 'tcp') . '://' . $host;
        if ($this->connected_to[0] != $host || $this->connected_to[1] != $uri->getPort()) {
            throw new Sabel_Exception_Runtime('Trying to write but we are connected to the wrong host');
        }
    ...
    public function read()
    {
        ...
        if (Sabel_Http_Response::extractCode($response) == 100 ||
            Sabel_Http_Response::extractCode($response) == 101) return $this->read();

        // If this was a HEAD request, return after reading the header (no need to read body)
        if ($this->method == Sabel_Http_Client::HEAD) return $response;
            ...
            if ($headers['transfer-encoding'] == 'chunked') {
                do {
                    ...
                    if (dechex($chunksize) != $hexchunksize) {
                        @fclose($this->socket);
                        throw new Sabel_Exception_Runtime('Invalid chunk size "' .
                ...
                throw new Sabel_Exception_Runtime('Cannot handle "' .
