 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Sabel_Http_Client  /* Zend_Http_Client */
{
    /**
     ...
     *
     * @var Sabel_Http_Client_Adapter_Interface
     */
     ...
     *
     * @param Sabel_Http_Uri|string $uri
     ...
     *
     * @param  Sabel_Http_Uri|string $uri
     * @return Sabel_Http_Client
     * @throws Sabel_Exception_Runtime
     */
    public function setUri($uri)
    {
        if (is_string($uri)) {
            $uri = new Sabel_Http_Uri($uri);
        }

        if (!$uri instanceof Sabel_Http_Uri) {
            throw new Sabel_Exception_Runtime('Passed parameter is not a valid HTTP URI.');
        }
     ...
     * @return Sabel_Http_Uri|string
     */
    public function getUri($as_string = false)
    {
        if ($as_string && $this->uri instanceof Sabel_Http_Uri) {
    ...
    {
        if (! is_array($config)) {
            throw new Sabel_Exception_Runtime('Expected array parameter, given ' . gettype($config));
        }
    ...
    public function setMethod($method = self::GET)
    {
        if (! preg_match('/^[A-Za-z_]+$/', $method)) {
            throw new Sabel_Exception_Runtime("'{$method}' is not a valid HTTP request method.");
        }
    ...
    public function setHeaders($name, $value = null)
    {
        ...
        if (is_array($name)) {
            ...
            if ($this->config['strict'] && (! preg_match('/^[a-zA-Z0-9-]+$/', $name))) {
                throw new Sabel_Exception_Runtime("{$name} is not a valid HTTP header name");
            }
     ...
     * <code>
     * $this->setAuth('shahar', 'secret', Sabel_Http_Client::AUTH_BASIC);
     * </code>
     *
    ...
    public function setAuth($user, $password = '', $type = self::AUTH_BASIC)
    {
        ...
        // Else, set up authentication
            ...
            if (! defined('self::AUTH_' . strtoupper($type))) {
                throw new Sabel_Exception_Runtime("Invalid or not supported authentication type: '$type'");
            }
     ...
     * and responses.
     *
     * @param Sabel_Http_CookieJar|boolean $cookiejar Existing cookiejar object, true to create a new one, false to disable
     * @return Sabel_Http_Client
     */
    public function setCookieJar($cookiejar = true)
    {
        if (! class_exists('Sabel_Http_CookieJar'))
        ...
        if ($cookiejar instanceof Sabel_Http_CookieJar) {
            ...
            throw new Sabel_Exception_Runtime('Invalid parameter type passed as CookieJar');
        }
     ...
     *
     * @return Sabel_Http_CookieJar|null
     */
    ...
    {
        if (! class_exists('Sabel_Http_Cookie'))
        ...
        if (isset($this->cookiejar)) {
            ...
            if (preg_match("/[=,; \t\r\n\013\014]/", $cookie)) {
                throw new Sabel_Exception_Runtime("Cookie name cannot contain these characters: =,; \t\r\n\013\014 ({$cookie})");
            }
    ...
    public function setFileUpload($filename, $formname, $data = null, $ctype = null)
    {
        if ($data === null) {
            if (($data = @file_get_contents($filename)) === false) {
                throw new Sabel_Exception_Runtime("Unable to read file '{$filename}' for upload");
            }
     ...
     *
     * @return Sabel_Http_Response or null if none
     */
     ...
     *
     * @param Sabel_Http_Client_Adapter_Interface|string $adapter
     */
    public function setAdapter($adapter)
    {
        ...
        if (! $adapter instanceof Sabel_Http_Client_Adapter_Interface) {
            throw new Sabel_Exception_Runtime('Passed adapter is not a HTTP connection adapter');
        }
     ...
     *
     * @param string $method
     * @return Sabel_Http_Response
     */
    public function request($method = null)
    {
        if (! $this->uri instanceof Sabel_Http_Uri) {
            throw new Sabel_Exception_Runtime('No valid URI has been passed to the client');
        }
        ...
        do {
            ...
            if (! $response) {
                throw new Sabel_Exception_Runtime('Unable to read response, or response is empty');
            }
            ...
            if ($response->isRedirect() && ($location = $response->getHeader('location'))) {
                ...
                if (Sabel_Http_Uri::check($location)) {
    ...
    protected function prepare_headers()
    {
        ...
        if (isset($this->cookiejar)) {
            $cookstr = $this->cookiejar->getMatchingCookies($this->uri,
                true, Sabel_Http_CookieJar::COOKIE_STRING_CONCAT);
    ...
    protected function prepare_body()
    {
        ...
        if (count($this->paramsPost) > 0 || count($this->files) > 0) {
            switch($this->enctype) {
                ...
                default:
                    throw new Sabel_Exception_Runtime("Cannot handle content type '{$this->enctype}' automatically." .
