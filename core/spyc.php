...
class Spyc {
   ...
   *
   * The dump method, when supplied with an array, will do its bes
  ...
  /**
   * Load YAML into a PHP array from an instantiated objec
   *
   ...
   */
  function load($input) {
    // See what type of input we're talking abou
	...
	// Create a new node and get its inden
	$node         = new YAMLNode;
	...
	if ($this->_lastIndent == $node->indent) {
	  ...
	  // We don't know the parent of the node so we have to find i
	...
	if ($this->_inBlock === false) {
	  ...
	  // Add the node to the master lis
  ...
  * @access private
   ...
   * @access private
   * @return string
   * @param $array The array you want to conver
   ...
   * @return string
   * @param $value
   * @param $indent int The value of the inden
   */
  ...
  function _parseLine($line) {
	...
	$array[] = array();
      ...
      // If the key is in double quotes pull it ou
  ...
  function _nodeArrayizeData(&$node) {
    if (is_array($node->data) && $node->children == true) {
      ...
      // We've gathered all our children's data and are ready to use i
      $key = key($node->data);
      $key = empty($key) ? 0 : $key;
